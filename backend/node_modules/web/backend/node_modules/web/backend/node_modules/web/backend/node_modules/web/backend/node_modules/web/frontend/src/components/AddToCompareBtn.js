...
export default function AddToCompareBtn({ productId }) {
    ...
    return (_jsx("button", { onClick: handleToggleCompare, disabled: isLoading, className: `flex items-center justify-center gap-2 px-4 py-2 rounded transition-all duration-300 font-semibold text-sm ${inCompare
            ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-md'
            : 'bg-gray-200 text-gray-700 hover:bg-gray-300'} disabled:opacity-50 disabled:cursor-not-allowed`, title: inCompare ? 'Xóa khỏi so sánh' : 'Thêm vào so sánh', children: _jsx(ArrowLeftRight, { size: 18 }) }));
}
