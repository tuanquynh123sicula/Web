...
export const useGetSummaryReportQuery = () => useQuery({
    queryKey: ['admin-summary-report'],
    queryFn: async () => (await apiClient.get(`/api/admin/reports/summary`)).data,
});
// Helper function để thêm BOM cho UTF-8
const addUtf8Bom = (content) => {
    return '\uFEFF' + content;
};
...
export const useExportReport = () => {
    const exportOrders = async (startDate = null, endDate = null) => {
        try {
    ...
    const exportOrdersToExcel = async (startDate = null, endDate = null) => {
        try {
            const params = new URLSearchParams();
            if (startDate)
                params.append('startDate', startDate);
